      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 top-full mt-1 z-20 bg-white border border-gray-200 rounded-xl shadow-lg p-1 min-w-45">
            <button onClick={() => { setOpen(false); onShare('whatsapp'); }} className="flex items-center gap-2 w-full px-3 py-2 text-sm rounded-lg hover:bg-green-50 text-green-700">
              <MessageCircle className="w-4 h-4" /> WhatsApp
            </button>
            <button onClick={() => { setOpen(false); onShare('email'); }} className="flex items-center gap-2 w-full px-3 py-2 text-sm rounded-lg hover:bg-blue-50 text-blue-700">
              <Mail className="w-4 h-4" /> Email
            </button>
            <button onClick={() => { setOpen(false); onShare('native'); }} className="flex items-center gap-2 w-full px-3 py-2 text-sm rounded-lg hover:bg-gray-50 text-gray-700">
      ...
      <header className="sticky top-0 z-20 bg-white border-b shadow-sm">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 shrink-0">
            {doc && (
              <>
                <Button variant="outline" size="sm" onClick={downloadPdf} disabled={isGenerating} className="gap-1.5">
        ...
        {doc && !loading && (
          <div className="space-y-5">
            ...
            <div ref={contentRef} className="overflow-x-auto w-full flex justify-center py-4 bg-gray-50">
              <DocumentPages content={doc.content} type={doc.type as any} title={doc.title} showPageNumbers={true}
...
export default function SuperviseurDocViewerPage() {
  return (
