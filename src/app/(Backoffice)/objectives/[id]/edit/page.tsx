...
export default function EditObjectivePage() {
  ...
  // ─── Chargement de l'objectif existant ─────────────────────────────────────
  useEffect(() => {
    if (!id) return;
    fetch(`/api/objectives/${id}`)
      ...
      .then(d => {
        if (d.objective) {
          ...
          setPeriodType(obj.periodType);
          setStartDate(obj.startDate ? new Date(obj.startDate).toISOString().split('T')[0] : '');
          setEndDate(obj.endDate ? new Date(obj.endDate).toISOString().split('T')[0] : '');
          setTitle(obj.title || '');
          setDescription(obj.description || '');
          setInheritedFromId(obj.inheritedFromId);
          
          if (obj.plans && obj.plans.length > 0) {
            const loadedPlans = obj.plans.map((p: any) => ({
              id: p.id,
              name: p.name,
              expanded: true,
              tasks: p.tasks.map((t: any) => ({
                ...
                teamId: t.teamId || '',
                score: t.score || 0,
                evaluationNote: t.evaluationNote || null,
                hasBeenEvaluated: t.hasBeenEvaluated || false
              }))
            }));
          ...
          <CardContent className="space-y-5">
            ...
            >
              {submitting
                ...
                : <><Save className="mr-2 h-4 w-4" />Mettre à jour l'objectif</>
              }
            </Button>
          </CardContent>
        </Card>
