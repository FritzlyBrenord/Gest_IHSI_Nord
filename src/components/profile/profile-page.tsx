...
export function ProfilePage({ fallbackRoute, headingLabel = 'Profil utilisateur' }: ProfilePageProps) {
  ...
  const displayName = profile?.name || headingLabel;
  ...
  const submit = async () => {
    ...
    try {
      ...
      setProfile((current) =>
        current
          ? { ...current, email: data.profile.email, name: data.profile.name, photoUrl: data.profile.photoUrl }
          : current,
      );
      setPhotoPreview(data.profile.photoUrl);
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                ...
                <Badge className={profile.canEdit ? 'bg-emerald-100 text-emerald-700 hover:bg-emerald-100' : 'bg-slate-100 text-slate-600 hover:bg-slate-100'}>
                  {profile.canEdit ? 'Compte modifiable' : 'Lecture seule'}
                </Badge>
              </div>
            </div>
          </div>
